...
bool parse_ruuvi_data_byte(const esp32_ble_tracker::adv_data_t &adv_data, RuuviParseResult &result) {
  ...
  switch (data_type) {
    case 0x03: {  // RAWv1
      ...
      const float equilibrium_vapor_pressure = 611.2f * exp(17.67f * temperature / (243.5f + temperature));
      const float air_density = 1.2929f * 273.15f / (temperature + 273.15f) * (pressure*100 - 0.3783f * humidity / 100.0f * equilibrium_vapor_pressure) / 101300.0f;
      const float absolute_humidity = equilibrium_vapor_pressure * humidity * 0.021674f / (273.15f + temperature);
      const double v = log(humidity / 100 * equilibrium_vapor_pressure / 611.2);
      const float dew_point = -243.5 * v / (v - 17.67);
      ...
      result.equilibrium_vapor_pressure = equilibrium_vapor_pressure;
      result.air_density = air_density;
      result.absolute_humidity = absolute_humidity;
    ...
    case 0x05: {  // RAWv2
      ...
      const float absolute_humidity = equilibrium_vapor_pressure * humidity * 0.021674f / (273.15f + temperature);
      const float acceleration_total = sqrt(acceleration_x * acceleration_x + acceleration_y * acceleration_y + acceleration_z * acceleration_z);
      const double v = log(humidity / 100 * equilibrium_vapor_pressure / 611.2);
      const float dew_point = -243.5 * v / (v - 17.67);
...
bool RuuviListener::parse_device(const esp32_ble_tracker::ESPBTDevice &device) {
  ...
  if (res->equilibrium_vapor_pressure.has_value()) {
    ESP_LOGD(TAG, "  Equilibrium Vapor Pressure: %.2f", *res->equilibrium_vapor_pressure);
  }
  if (res->air_density.has_value()) {
    ESP_LOGD(TAG, "  Air Density: %.6f", *res->air_density);
  }
  if (res->absolute_humidity.has_value()) {
    ESP_LOGD(TAG, "  Absolute humidity: %.2f", *res->absolute_humidity);
  }
  if (res->dew_point.has_value()) {
    ESP_LOGD(TAG, "  Dew Point: %.2f", *res->dew_point);
  }
