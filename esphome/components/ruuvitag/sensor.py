    ...
    cv.Schema(
        {
            ...
            cv.Optional("equilibrium_vapor_pressure"): sensor.sensor_schema(
                unit_of_measurement=UNIT_HECTOPASCAL,
                accuracy_decimals=2,
                device_class=DEVICE_CLASS_PRESSURE,
                state_class=STATE_CLASS_MEASUREMENT,
            ),
            cv.Optional("air_density"): sensor.sensor_schema(
                unit_of_measurement="kg/m³",
                accuracy_decimals=6,
                device_class=DEVICE_CLASS_PRESSURE,
                state_class=STATE_CLASS_MEASUREMENT,
            ),
            cv.Optional("absolute_humidity"): sensor.sensor_schema(
...
async def to_code(config):
    ...
    if "equilibrium_vapor_pressure" in config:
        sens = await sensor.new_sensor(config["equilibrium_vapor_pressure"])
        cg.add(var.set_equilibrium_vapor_pressure(sens))
    if "air_density" in config:
        sens = await sensor.new_sensor(config["air_density"])
        cg.add(var.set_air_density(sens))
    if "absolute_humidity" in config:
        sens = await sensor.new_sensor(config["absolute_humidity"])
        cg.add(var.set_absolute_humidity(sens))
    if "dew_point" in config:
        sens = await sensor.new_sensor(config["dew_point"])
        cg.add(var.set_dew_point(sens))