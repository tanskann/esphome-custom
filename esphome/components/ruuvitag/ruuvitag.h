...
class RuuviTag : public Component, public esp32_ble_tracker::ESPBTDeviceListener {
 public:
  ...
  bool parse_device(const esp32_ble_tracker::ESPBTDevice &device) override {
    ...
    if (res->equilibrium_vapor_pressure.has_value() && this->equilibrium_vapor_pressure_ != nullptr)
      this->equilibrium_vapor_pressure_->publish_state(*res->equilibrium_vapor_pressure);
    if (res->air_density.has_value() && this->air_density_ != nullptr)
      this->air_density_->publish_state(*res->air_density);
    if (res->absolute_humidity.has_value() && this->absolute_humidity_ != nullptr)
      this->absolute_humidity_->publish_state(*res->absolute_humidity);
    if (res->dew_point.has_value() && this->dew_point_ != nullptr)
      this->dew_point_->publish_state(*res->dew_point);
    return true;
  }
  ...
  void set_equilibrium_vapor_pressure(sensor::Sensor *equilibrium_vapor_pressure) { equilibrium_vapor_pressure_ = equilibrium_vapor_pressure; }
  void set_air_density(sensor::Sensor *air_density) { air_density_ = air_density; }
  void set_absolute_humidity(sensor::Sensor *absolute_humidity) { absolute_humidity_ = absolute_humidity; }
  void set_dew_point(sensor::Sensor *dew_point) { dew_point_ = dew_point; }
 ...
 protected:
  ...
  sensor::Sensor *equilibrium_vapor_pressure_{nullptr};
  sensor::Sensor *air_density_{nullptr};
